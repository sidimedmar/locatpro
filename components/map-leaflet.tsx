...
export default function MapLeaflet({ properties, onViewProperty }: MapLeafletProps) {
  const mapRef = useRef<HTMLDivElement>(null);
  const mapInstanceRef = useRef<L.Map | null>(null);

  // Group properties by location
  const markers = useMemo(() => {
    const groups: Record<string, { lat: number; lng: number; properties: Property[]; wilaya: string; moughataa: string }> = {};
    properties.forEach((p) => {
      const key = `${p.wilaya}|${p.moughataa}`;
      if (!groups[key]) {
        const coords = getPropertyCoordinates(p.wilaya, p.moughataa);
        if (coords) {
          groups[key] = { ...coords, properties: [], wilaya: p.wilaya, moughataa: p.moughataa };
        }
      }
      if (groups[key]) {
  ...
  useEffect(() => {
    ...
    // Add wilaya center markers (empty wilayas)
    Object.entries(WILAYA_COORDINATES).forEach(([, wilayaGeo]) => {
      L.circleMarker([wilayaGeo.center.lat, wilayaGeo.center.lng], {
        radius: 3,
        color: "#94a3b8",
        fillColor: "#cbd5e1",
        fillOpacity: 0.4,
        weight: 1,
      }).addTo(map).bindTooltip(wilayaGeo.nameAr, {
        permanent: false,
        direction: "top",
    ...
    markers.forEach((group) => {
      ...
      const totalRent = group.properties.reduce((s, p) => s + Number(p.monthlyRent || 0), 0);
      const arrears = group.properties.reduce((s, p) => s + Number(p.arrears || 0), 0);
      ...
      const icon = L.divIcon({
        ...
        iconAnchor: [(24 + count * 4) / 2, (24 + count * 4) / 2],
      });
      ...
      const popupContent = `
        <div style="min-width:250px;max-width:320px;font-family:Cairo,sans-serif;direction:rtl">
      ...
      // Handle click on property items in popup
      marker.on("popupopen", () => {
        setTimeout(() => {
          const items = document.querySelectorAll(".property-popup-item");
          items.forEach((item) => {
            item.addEventListener("click", () => {
              const id = (item as HTMLElement).dataset.id;
              const prop = group.properties.find((p) => p.id === id);
