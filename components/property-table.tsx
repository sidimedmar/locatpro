...
import { Droplet, Zap, Phone, User, Trash2, MapPin, Eye, Pencil } from "lucide-react";
  ...
  if (properties.length === 0) {
    return (
      <div className="bg-card text-card-foreground p-12 rounded-2xl shadow-sm text-center border-2 border-dashed border-border">
        ...
        <h3 className="text-lg font-bold text-muted-foreground">{"لا توجد سجلات حالياً"}</h3>
            ...
            {properties.map((p) => (
              <tr key={p.id} className="hover:bg-muted/50 transition-colors">
                <td className="px-6 py-4">
                  <div className="font-bold text-card-foreground">{p.wilaya} - {p.moughataa}</div>
                  <div className="text-sm text-muted-foreground">{p.neighborhood}، {"منزل"} {p.houseNumber}</div>
                </td>
                <td className="px-6 py-4">
                  <div className="flex items-center gap-2">
                    <User className="w-4 h-4 text-primary" />
                    <span className="font-semibold text-card-foreground">{p.tenantName}</span>
                  </div>
                  <div className="flex items-center gap-2 text-xs text-muted-foreground mt-1">
                    <Phone className="w-3 h-3" />{p.tenantPhone}
                  </div>
                </td>
                <td className="px-6 py-4">
                ...
                <td className="px-6 py-4">
                  <div className="font-bold text-primary">{Number(p.monthlyRent).toLocaleString()} {"أوقية"}</div>
                  <div className="text-xs text-muted-foreground">{p.paymentSystem}</div>
                  {Number(p.arrears) > 0 && (
                    <div className="text-xs text-destructive font-bold mt-1">{"متأخرات:"} {Number(p.arrears).toLocaleString()}</div>
                  )}
                </td>
                <td className="px-6 py-4">
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-1">
                    <button onClick={() => onView(p)} className="p-2 hover:bg-primary/10 text-primary rounded-lg transition-colors" title="عرض التفاصيل">
                      <Eye className="w-4 h-4" />
                    </button>
                    <button onClick={() => onEdit(p)} className="p-2 hover:bg-secondary/20 text-secondary-foreground rounded-lg transition-colors" title="تعديل">
                      <Pencil className="w-4 h-4" />
                    </button>
                    <button
                      onClick={() => { if (confirm("هل أنت متأكد من حذف هذا السجل؟")) { onDelete(p.id); } }}
                      className="p-2 hover:bg-destructive/10 text-destructive rounded-lg transition-colors" title="حذف"
                    >
